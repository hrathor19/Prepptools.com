  ...
  if (allPosts.length === 0) {
    return (
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <h1 className="text-3xl font-extrabold text-gray-900 dark:text-white mb-2">Articles</h1>
    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      ...
      {featuredPost && (
        <section className="mb-12">
          <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-4">Featured Article</h2>

          <Link
            href={`/blog/${featuredPost.slug}`}
            className="group block bg-white dark:bg-gray-800 rounded-2xl overflow-hidden shadow-sm border border-gray-100 dark:border-gray-700 hover:shadow-lg dark:hover:border-gray-600 transition-all duration-200"
          >
            <div className="flex flex-col md:flex-row">
              ...
              <div className="relative md:w-[55%] flex-shrink-0 h-64 md:h-auto overflow-hidden">
                {featuredPost.coverImageUrl ? (
                  ...
                  <div className="flex items-center justify-center bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-blue-900/30 dark:to-indigo-900/30 text-8xl h-full min-h-[260px]">
              ...
              <div className="flex flex-col justify-center p-8 md:p-10 flex-1">
                {featuredPost.tags[0] && (
                  <span className="inline-block self-start text-xs font-semibold text-blue-600 dark:text-blue-400 bg-blue-50 dark:bg-blue-900/30 border border-blue-100 dark:border-blue-800 px-3 py-1 rounded-full mb-4">
                    {featuredPost.tags[0]}
                  </span>
                )}

                <h3 className="text-2xl font-extrabold text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors leading-snug mb-3">
                  {featuredPost.title}
                </h3>
      ...
      {pagePosts.length > 0 && (
        <section>
          <h2 className="text-lg font-bold text-gray-900 dark:text-white mb-6">Latest Articles</h2>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            {pagePosts.map((post) => (
              <Link
                key={post.slug}
                href={`/blog/${post.slug}`}
                className="group bg-white dark:bg-gray-800 rounded-2xl overflow-hidden border border-gray-100 dark:border-gray-700 shadow-sm hover:shadow-lg dark:hover:border-gray-600 transition-all duration-200 flex flex-col"
              >
                {/* Image */}
                <div className="relative h-48 overflow-hidden flex-shrink-0">
                  {post.coverImageUrl ? (
                    ...
                    <div className="flex items-center justify-center bg-gradient-to-br from-gray-50 to-gray-100 dark:from-gray-700 dark:to-gray-800 h-full text-6xl">
                      {post.coverEmoji}
                    </div>
                  )}
                  {post.tags[0] && (
                ...
                <div className="p-5 flex flex-col flex-1">
                  <h3 className="font-bold text-gray-900 dark:text-white group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors mb-2 line-clamp-2 leading-snug">
                    {post.title}
                  </h3>
      ...
      {regularPosts.length === 0 && !featuredPost && (
        <p className="text-gray-400 dark:text-gray-500 text-sm">No articles available.</p>
      )}
          ...
          <div className="flex items-center gap-2">
            {safePage > 1 ? (
              <Link
                ...
                className="flex items-center gap-1 px-4 py-2 text-sm text-gray-600 dark:text-gray-300 bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg hover:border-blue-400 hover:text-blue-600 dark:hover:text-blue-400 transition-all"
              >
