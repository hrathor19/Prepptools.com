...
import { PenSquare, Eye, EyeOff, PlusCircle, Heart, ArrowLeft } from "lucide-react";
          ...
          <table className="w-full text-sm">
            <thead className="bg-gray-50 border-b border-gray-200">
              <tr>
                ...
                <th className="text-center px-4 py-3 font-semibold text-gray-600 hidden md:table-cell"><Heart className="w-3.5 h-3.5 inline" /></th>
            ...
            <tbody className="divide-y divide-gray-100">
              {posts.map((post) => (
                <tr key={post.id} className="hover:bg-gray-50 transition-colors">
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      {post.coverImageUrl ? (
                        ...
                        <img src={post.coverImageUrl} alt="" className="w-10 h-10 rounded-lg object-cover shrink-0" />
                      ) : (
                        <span className="text-xl w-10 h-10 flex items-center justify-center bg-gray-100 rounded-lg shrink-0">{post.coverEmoji}</span>
                      )}
                  ...
                  <td className="px-4 py-4 text-center text-gray-500 hidden md:table-cell">{post.likes}</td>
                  <td className="px-5 py-4">
